...
def forward_skip_vs_fixed(model, input_ids, tokenizer, collector_skip, start_layer, end_layer):
    seq_len = input_ids.shape[1]
    position_ids = torch.arange(seq_len, dtype=torch.long, device=input_ids.device).unsqueeze(0)
    hidden_states = model.model.embed_tokens(input_ids)
    ...
    return next_token_id, token_str, input_ids, end_layer - start_layer, total_time
